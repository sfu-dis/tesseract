...
class mbtree {
 public:
  ...
  inline void search_amac(std::vector<AMACState> &states,
                          epoch_num epoch) const;

  inline ermia::coro::generator<bool> search_coro(
      const key_type &k, OID &o, threadinfo &ti,
      versioned_node_t *search_info = nullptr) const;
   ...
   */
  PROMISE(void)
  search_range_call(const key_type &lower, const key_type *upper,
                    low_level_search_range_callback &callback,
                    TXN::xid_context *xc) const;

  // (lower, upper]
  PROMISE(void)
  rsearch_range_call(const key_type &upper, const key_type *lower,
                     low_level_search_range_callback &callback,
                     TXN::xid_context *xc) const;
  ...
  template <typename F>
  inline PROMISE(void)
      search_range(const key_type &lower, const key_type *upper, F &callback,
                   TXN::xid_context *xc) const;
  ...
  template <typename F>
  inline PROMISE(void)
      rsearch_range(const key_type &upper, const key_type *lower, F &callback,
                    TXN::xid_context *xc) const;
   ...
   */
  inline PROMISE(bool)
      insert(const key_type &k, OID o, TXN::xid_context *xc,
             value_type *old_oid = NULL, insert_info_t *insert_info = NULL);
   ...
   */
  inline PROMISE(bool)
      insert_if_absent(const key_type &k, OID o, TXN::xid_context *xc,
 ...
 public:
  ...
  template <bool Reverse>
  class low_level_iterator_scanner;
  template <typename F>
  ...
  class ScanIterator {
   ...
   public:
    ...
    static PROMISE(ScanIterator<IsReverse>)
        factory(mbtree<P> *mbtree, ermia::TXN::xid_context *xc,
                const ermia::varstr &start_key, const ermia::varstr *end_key,
      ...
      while (1) {
        si.state = AWAIT si.stack[si.stackpos].find_initial(
            scan_iterator.helper_, si.ka, emit_firstkey, si.entry, ti);
        scanner.visit_leaf(si.stack[si.stackpos], si.ka, ti);
        if (si.state != Masstree::scan_info<P>::mystack_type::scan_down) break;
        si.ka.shift();
        ++si.stackpos;
      }
  ...
  class coro_ScanIterator {
   ...
   public:
    ...
    ermia::coro::generator<bool> init(bool emit_firstkey = true) {
      ...
      while (1) {
        ...
        __reach_leaf_retry:
          ...
          while (!v[sense].isleaf()) {
            const Masstree::internode<P> *in =
                static_cast<const Masstree::internode<P> *>(n[sense]);
            in->prefetch();
          ...
          s.v_ = v[sense];
          s.n_ = const_cast<Masstree::leaf<P> *>(
              static_cast<const Masstree::leaf<P> *>(n[sense]));
        }

        find_initial_retry_node:
          if (s.v_.deleted()) goto find_initial_retry_root;
          s.n_->prefetch();
          co_await suspend_always{};
          ...
          if (kp >= 0) {
            if (s.n_->keylenx_is_layer(keylenx)) {
              ...
              if (this->helper_.initial_ksuf_match(ksuf_compare,
                                                   emit_firstkey)) {
      ...
      // si.state = AWAIT
      // si.stack[si.stackpos].find_initial(scan_iterator.helper_, si.ka,
      // emit_firstkey, si.entry, ti);
      find_initial_done:
        scanner.visit_leaf(si.stack[si.stackpos], si.ka, ti);
        if (si.state != Masstree::scan_info<P>::mystack_type::scan_down) break;
        si.ka.shift();
      ...
      while (1) {
        switch (si.state) {
          ...
          find_next:
            si.state =
                si.stack[si.stackpos].find_next(this->helper_, si.ka, si.entry);
          ...
          case Masstree::scan_info<P>::mystack_type::scan_up:
            do {
              if (--si.stackpos < 0) co_return false;
              si.ka.unshift();
              si.stack[si.stackpos].ki_ =
                  this->helper_.next(si.stack[si.stackpos].ki_);
          ...
          retry:
            si.state = AWAIT si.stack[si.stackpos].find_retry(this->helper_,
                                                              si.ka, ti);
            break;
        }
      }
...
template <typename P>
inline void mbtree<P>::search_amac(std::vector<AMACState> &states,
                                   epoch_num epoch) const {
  threadinfo ti(epoch);
  uint32_t todo = states.size();
  for (auto &s : states)
    s.lp = Masstree::unlocked_tcursor<P>(table_, s.key->data(), s.key->size());
          ...
          if (!s.n[!s.sense]) {
            ...
            if (likely(!in->has_changed(s.v[s.sense]))) {
              ...
              if (s.v[s.sense].isleaf()) {
                s.lp.v_ = s.v[s.sense];
                s.lp.n_ = const_cast<Masstree::leaf<P> *>(
                    static_cast<const Masstree::leaf<P> *>(s.n[s.sense]));
                if (s.lp.v_.deleted()) {
                  s.stage = 0;
                  goto stage0;
                } else {
              ...
              typename Masstree::node_base<P>::nodeversion_type oldv =
                  s.v[s.sense];
              s.v[s.sense] = in->stable_annotated(ti.stable_fence());
              if (oldv.has_split(s.v[s.sense]) &&
                  in->stable_last_key_compare(s.lp.ka_, s.v[s.sense], ti) > 0) {
                s.stage = 0;
                goto stage0;
              } else {
                if (s.v[s.sense].isleaf()) {
                  ...
                  Masstree::internode<P> *in =
                      (Masstree::internode<P> *)s.n[s.sense];
                  in->prefetch();
                  s.ptr = in;
                  assert(s.stage == 1);
                }
...
template <typename P>
inline ermia::coro::generator<bool> mbtree<P>::search_coro(
    const key_type &k, OID &o, threadinfo &ti,
    versioned_node_t *search_info) const {
  Masstree::unlocked_tcursor<P> lp(table_, k.data(), k.size());
...
retry2:
  ...
  lp.v_ = v[sense];
  lp.n_ = const_cast<Masstree::leaf<P> *>(
      static_cast<const Masstree::leaf<P> *>(n[sense]));
...
template <typename P>
inline PROMISE(bool) mbtree<P>::insert(const key_type &k, OID o,
                                       TXN::xid_context *xc,
                                       value_type *old_oid,
...
class mbtree<P>::low_level_iterator_scanner
    : public search_range_scanner_base<Reverse> {
 public:
  low_level_iterator_scanner(const mbtree<P> *btr_ptr, const key_type *boundary)
...
template <typename P>
inline PROMISE(void) mbtree<P>::search_range_call(
    const key_type &lower, const key_type *upper,
  ...
  threadinfo ti(xc->begin_epoch);
  AWAIT table_.scan(lcdf::Str(lower.data(), lower.size()), true, scanner, xc,
                    ti);
}

template <typename P>
inline PROMISE(void) mbtree<P>::rsearch_range_call(
    const key_type &upper, const key_type *lower,
    low_level_search_range_callback &callback, TXN::xid_context *xc) const {
  low_level_search_range_scanner<true> scanner(this, lower, callback);
  threadinfo ti(xc->begin_epoch);
  AWAIT table_.rscan(lcdf::Str(upper.data(), upper.size()), true, scanner, xc,
                     ti);
}
