...
// TODO: support scan_min length, current zipfain rng does not support min
// bound.
...
void ycsb_parse_options(int argc, char **argv) {
  ...
  while (1) {
    ...
    int option_index = 0;
    int c =
        getopt_long(argc, argv, "r:a:w:s:z:t:g:", long_options, &option_index);
    if (c == -1) break;
    switch (c) {
      case 0:
  ...
  if (ermia::config::verbose) {
    ...
    if (g_read_txn_type == ReadTransactionType::Sequential) {
      ...
      std::cerr << "  read transaction type:      simple coroutine multi-get"
                << std::endl;
    } else if (g_read_txn_type == ReadTransactionType::AdvCoroMultiGet) {
      std::cerr << "  read transaction type:      advanced coroutine multi-get"
                << std::endl;
    } else if (g_read_txn_type == ReadTransactionType::AdvCoro) {
      std::cerr << "  read transaction type:      advanced coroutine"
                << std::endl;
    } else if (g_read_txn_type == ReadTransactionType::SimpleCoro) {
      std::cerr << "  read transaction type:      simple coroutine"
                << std::endl;
    } else {
      abort();
    }
    ...
    if (ycsb_workload.scan_percent() > 0) {
      if (g_scan_max_length < g_scan_min_length || g_scan_min_length < 1) {
        ...
        std::cerr << "  min :                    " << g_scan_min_length
                  << std::endl;
        std::cerr << "  max :                    " << g_scan_max_length
                  << std::endl;
      }
      std::cerr << "  scan maximal range:         " << g_scan_max_length
                << std::endl;
    }
  }
}
