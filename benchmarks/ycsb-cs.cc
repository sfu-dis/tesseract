...
class ycsb_cs_worker : public ycsb_base_worker {
 public:
  ...
  virtual workload_desc_vec get_workload() const override {
    ...
    LOG_IF(FATAL, g_read_txn_type != ReadTransactionType::SimpleCoro)
        << "Read txn type must be simple-coro";
  ...
  static ermia::coro::generator<rc_t> TxnRead(bench_worker *w, uint32_t idx,
                                              ermia::epoch_num begin_epoch) {
    return static_cast<ycsb_cs_worker *>(w)->txn_read(idx, begin_epoch);
  }

  static ermia::coro::generator<rc_t> TxnRMW(bench_worker *w, uint32_t idx,
                                             ermia::epoch_num begin_epoch) {
    return static_cast<ycsb_cs_worker *>(w)->txn_rmw(idx, begin_epoch);
  }

  static ermia::coro::generator<rc_t> TxnScan(bench_worker *w, uint32_t idx,
                                              ermia::epoch_num begin_epoch) {
    return static_cast<ycsb_cs_worker *>(w)->txn_scan(idx, begin_epoch);
  }

  static ermia::coro::generator<rc_t> TxnScanWithIterator(
      bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<ycsb_cs_worker *>(w)->txn_scan_with_iterator(
        idx, begin_epoch);
  }

  // Read transaction with context-switch using simple coroutine
  ermia::coro::generator<rc_t> txn_read(uint32_t idx,
                                        ermia::epoch_num begin_epoch) {
    ...
    for (int i = 0; i < g_reps_per_tx; ++i) {
      ...
      if (ermia::config::index_probe_only) {
        ...
        new (&k) ermia::varstr((char *)&k + sizeof(ermia::varstr),
                               sizeof(ycsb_kv::key));
        ...
        rc._val = (co_await table_index->GetMasstree().search_coro(k, oid, ti,
                                                                   &sinfo))
                      ? RC_TRUE
                      : RC_FALSE;
      } else {
  ...
  ermia::coro::generator<rc_t> txn_rmw(uint32_t idx,
                                       ermia::epoch_num begin_epoch) {
    auto *txn = db->NewTransaction(ermia::transaction::TXN_FLAG_CSWITCH,
                                   arenas[idx], &transactions[idx], idx);
    ermia::TXN::xid_context *xc = txn->GetXIDContext();
    xc->begin_epoch = begin_epoch;
  ...
  ermia::coro::generator<rc_t> txn_scan(uint32_t idx,
                                        ermia::epoch_num begin_epoch) {
    auto *txn = db->NewTransaction(ermia::transaction::TXN_FLAG_CSWITCH |
                                       ermia::transaction::TXN_FLAG_READ_ONLY,
                                   arenas[idx], &transactions[idx], idx);
    ...
    for (int i = 0; i < g_reps_per_tx; ++i) {
      ...
      rc = co_await table_index->coro_Scan(txn, range.start_key, &range.end_key,
                                           callback);
  ...
  ermia::coro::generator<rc_t> txn_scan_with_iterator(
    ...
    for (int i = 0; i < g_reps_per_tx; ++i) {
      ...
      ScanRange range = GenerateScanRange(txn);
      ycsb_scan_callback callback;
      ermia::ConcurrentMasstree::coro_ScanIterator</*IsRerverse=*/false> iter(
          txn->GetXIDContext(), &table_index->GetMasstree(), range.start_key,
          &range.end_key);
