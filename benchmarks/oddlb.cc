...
class oddlb_sequential_worker : public oddlb_base_worker {
 public:
  oddlb_sequential_worker(unsigned int worker_id, unsigned long seed, ermia::Engine *db,
  ...
  static rc_t TxnDDL(bench_worker *w) { return static_cast<oddlb_sequential_worker *>(w)->txn_ddl(); }
  static rc_t TxnRead(bench_worker *w) { return static_cast<oddlb_sequential_worker *>(w)->txn_read(); }
  static rc_t TxnRMW(bench_worker *w) { return static_cast<oddlb_sequential_worker *>(w)->txn_rmw(); }
  ...
  rc_t txn_ddl() {
    ...
    ermia::ConcurrentMasstreeIndex *old_table_index = (ermia::ConcurrentMasstreeIndex *) schema.index; 
    ...
    if (rc._val != RC_TRUE) TryCatchUnblock(rc);
...
retry:
...
#endif    
    uint64_t a = rand() % oddl_initial_table_size; // 0 ~ oddl_initial_table_size-1
  ...
  retry:
    ...
    if (schema_version == 0) {
      ...
      table_index->GetRecord(txn, rc, k2, v2, &oid);
      if (rc._val != RC_TRUE) {
        ermia::TableDescriptor *old_table_descriptor = (ermia::TableDescriptor *) schema.old_td;
