...
static const CSN INVALID_CSN = ~uint64_t{0};
static const LSN INVALID_LSN = ~uint64_t{0};
...
// this structure. Upon commit we only deal with log blocks and copy log blocks
// in their entirety to the log buffer from the transaction's scratch area.
struct log_block {
  ...
  log_block() : csn(INVALID_CSN), payload_size(0) {}
  ~log_block() {}
...
public:
  ...
  void initialize(const char *log_dir, int log_id, int node, uint32_t logbuf_mb);
  void uninitialize();
