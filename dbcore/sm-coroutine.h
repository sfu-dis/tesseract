  ...
  void *alloc(size_t byte_size) {
    ...
    if (frame_to_alloc == nullptr) {
      ...
      frame_to_alloc = reinterpret_cast<FrameNode *>(
          alloc_from_arena(sizeof(FrameNode) + frame_size, CACHELINE_SIZE));
...
template <>
struct generator<void>::promise_type {
  promise_type() {}
  ~promise_type() {}
  auto get_return_object() { return generator{handle::from_promise(*this)}; }
  auto initial_suspend() { return suspend_never{}; }
...
struct promise_base {
  ...
  auto final_suspend() noexcept {
    ...
    if (!parent_) {
      return coro_task_private::final_awaiter(noop_coroutine());
    }

    ASSERT(root_);
    root_->leaf_ = parent_;
    return coro_task_private::final_awaiter(parent_->get_coro_handle());
  }
  void unhandled_exception() { std::terminate(); }
  ...
  inline generic_coroutine_handle get_coro_handle() const { return handle_; }
...
struct task<T>::promise_type : coro_task_private::promise_base {
  using coroutine_handle_t = coroutine_handle<typename task<T>::promise_type>;
  ...
  void return_value(T value) { new (&ret_val_buf_) T(std::move(value)); }
  T &&transfer_return_value() {
    return std::move(*reinterpret_cast<T *>(&ret_val_buf_));
  }
...
struct task<T>::awaiter {
  ...
  auto await_suspend(
      coroutine_handle<awaiting_promise_t> awaiting_coroutine) noexcept {
    suspended_task_coroutine_.promise().set_parent(
