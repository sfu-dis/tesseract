      ...
      if (config::amac_version_chain) {
        ...
        for (uint32_t i = 0; i < requests.size(); ++i) {
          ...
          if (r.out_oid != INVALID_OID) {
            auto *tuple = oidmgr->oid_get_version(
                table_descriptor->GetTupleArray(), r.out_oid, t->xc);
            if (tuple) {
...
void ConcurrentMasstreeIndex::simple_coro_MultiGet(
  ...
  if (!t) {
    ...
    for (int i = 0; i < keys.size(); ++i) {
      handles[i] =
          masstree_.search_coro(*keys[i], oid, ti, &sinfo).get_handle();
    }
  } else {
    for (int i = 0; i < keys.size(); ++i) {
  ...
  if (!t) MM::epoch_exit(0, e);
}

ermia::coro::generator<rc_t> ConcurrentMasstreeIndex::coro_GetRecordSV(
    transaction *t, const varstr &key, varstr &value, OID *out_oid) {
  OID oid = INVALID_OID;
...
ermia::coro::generator<rc_t> ConcurrentMasstreeIndex::coro_GetRecord(
    transaction *t, const varstr &key, varstr &value, OID *out_oid) {
  OID oid = INVALID_OID;
  rc_t rc = rc_t{RC_INVALID};
  t->ensure_active();
...
ermia::coro::generator<rc_t> ConcurrentMasstreeIndex::coro_UpdateRecord(
    transaction *t, const varstr &key, varstr &value) {
...
forward:
  ...
  sinfo =
      ConcurrentMasstree::versioned_node_t(lp.node(), lp.full_version_value());
  // end: masstree search

  if (match) {
    // By default we don't do coroutine prefetch-yield here for updates,
    // assuming it's part of an RMW (most cases) which means the data is
    // probably in cache anyway. This may not be true however for blind updates.
  ...
  check_prev:
    ...
    if (prev_obj) {  // succeeded
      ...
      if (prev_csn.asi_type() == fat_ptr::ASI_XID and
        ...
        // causing readers to not find any visible version. Fix this together
        // with GC later.
        // MM::deallocate(prev_obj_ptr);
      } else {  // prev is committed (or precommitted but in post-commit now)
                // head
      ...
      ASSERT(is_delete ||
             (value.ptr.offset() && value.ptr.asi_type() == fat_ptr::ASI_LOG));
      ...
      t->log->log_update(tuple_fid, oid,
...
ermia::coro::generator<bool> ConcurrentMasstreeIndex::coro_InsertOID(
    transaction *t, const varstr &key, OID oid) {
  ...
  epoch_num e = 0;
  if (t->xc) e = t->xc->begin_epoch;
  ConcurrentMasstree::threadinfo ti(e);
  ConcurrentMasstree::tcursor_type lp(*masstree_.get_table(), key.data(),
                                      key.size());
...
forward:
  ...
  if (lp.kx_.p >= 0) {
    ...
    if (lp.state_ < 0 && !lp.n_->has_changed(version) &&
        !lv.layer()->has_split()) {
  ...
  if (lp.n_->has_changed(version) || lp.n_->permutation() != perm) {
    ...
    lp.ka_.shift_by(-lp.state_);
    lp.n_->lv_[lp.kx_.p] = root =
        lp.n_->lv_[lp.kx_.p].layer()->unsplit_ancestor();
    lp.n_->unlock();
    goto retry;
  ...
  if (lp.state_) {
    ...
    if (lp.kx_.p >= 0) {
      ...
      if (unlikely(lp.n_->modstate_ !=
                   ConcurrentMasstree::leaf_type::modstate_insert)) {
        masstree_invariant(lp.n_->modstate_ ==
                           ConcurrentMasstree::leaf_type::modstate_remove);
        lp.n_->mark_insert();
      ...
      if (lp.n_->size() < lp.n_->width) {
        lp.kx_.p =
            ConcurrentMasstree::leaf_type::permuter_type(lp.n_->permutation_)
                .back();
        // don't inappropriately reuse position 0, which holds the ikey_bound
        if (likely(lp.kx_.p != 0) || !lp.n_->prev_ ||
            lp.n_->ikey_bound() == lp.ka_.ikey()) {
      ...
      if (found) found = lp.make_split(ti);
    }
  }
  // end: find_insert
...
ermia::coro::generator<rc_t> ConcurrentMasstreeIndex::coro_InsertRecord(
    transaction *t, const varstr &key, varstr &value, OID *out_oid) {
  // For primary index only
  ALWAYS_ASSERT(IsPrimary());
...
forward:
  ...
  if (!inserted) co_return rc_t{RC_ABORT_INTERNAL};
...
ermia::coro::generator<rc_t> ConcurrentMasstreeIndex::coro_Scan(
    transaction *t, const varstr &start_key, const varstr *end_key,
  ...
  ConcurrentMasstree::low_level_search_range_scanner<false> scanner(
      &masstree_, end_key ? end_key : nullptr, cb);
  ConcurrentMasstree::threadinfo ti(t->xc->begin_epoch);
  ...
  typedef typename ConcurrentMasstree::node_type::leaf_type::leafvalue_type
      leafvalue_type;
  union {
    ikey_type
  ...
  while (1) {
    ...
    __reach_leaf_retry:
      ...
      s.v_ = v[sense];
      s.n_ = const_cast<ConcurrentMasstree::leaf_type *>(
          static_cast<const ConcurrentMasstree::leaf_type *>(n[sense]));
    }
  ...
  while (1) {
    switch (state) {
      case mystack_type::scan_emit: {  // surpress cross init warning about v
        ...
        {
          fat_ptr *oid_entry =
              table_descriptor->GetTupleArray()->get(entry.value());
      ...
      __reach_leaf_retry2:
        ...
        while (!v[sense].isleaf()) {
          ...
          // co_await suspend_always{};
          int kp =
              ConcurrentMasstree::internode_type::bound_type::upper(ka, *in);
          n[!sense] = in->child_[kp];
